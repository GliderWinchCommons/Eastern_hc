...
import Communications.PccFinal;
...
public class LaunchParameterGenerator {
   ...
   public LaunchParameterGenerator()
   {
       ...
       */ 
       ...
       // creates a temporary pilot object  from the current scenario set
       Sailplane glider =  currentSet.getCurrentSailplane();
   ...
   {
       //temporary float arrray used for testing with paramters found in the PccFinal class
       /*
        float[] launchparameter_flt = new float[PccFinal.launchparameter_dbl.length ];
        for(int x  = 0; x < PccFinal.launchparameter_dbl.length; x++)
        {
            launchparameter_flt[x] = (float)PccFinal.launchparameter_dbl[x];
        }
        */
       
       float[] launchparameter_flt = new float[200];
       for(int x  = 0; x < launchparameter_flt.length; x++)
        {
            launchparameter_flt[x] = (float)0.1+x;
        }
       
        return launchparameter_flt;
   }
}
